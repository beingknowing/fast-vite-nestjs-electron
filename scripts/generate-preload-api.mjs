...
const methodsByChannel = new Map()
...
const sortedMethods = [...methodsByChannel.values()].sort((a, b) => a.channel.localeCompare(b.channel))
const methodNameUsed = new Set()
...
lines.push('// Generated by scripts/generate-preload-api.mjs. DO NOT EDIT.')
lines.push('')
lines.push("import { ipcRenderer } from 'electron'")
lines.push("import type { IpcChannel, IpcInvokeArgs, IpcInvokeMap, IpcInvokeReturn } from '../../types/ipc-methods'")
lines.push('')
lines.push('type PreloadInvokeReturn<T extends IpcChannel, TAsync extends boolean> =')
lines.push('  TAsync extends true ? IpcInvokeReturn<T> : IpcInvokeMap[T][\'return\']')
lines.push('')
lines.push('export const ipcInvoke = {')

for (const item of channelBindings) {
    lines.push(`  ${item.methodName}: (...args: IpcInvokeArgs<'${item.channel}'>): PreloadInvokeReturn<'${item.channel}', ${item.isAsync}> =>`)
    lines.push(`    ipcRenderer.invoke('${item.channel}', ...args) as unknown as PreloadInvokeReturn<'${item.channel}', ${item.isAsync}>,`)
}
